...
export async function uploadToInstagram(
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    let statusData: { status_code?: string } | null = null;
    try {
      const statusRes = await fetchWithTimeout(
        `${GRAPH_API}/${containerId}?fields=status_code&access_token=${accessToken}&appsecret_proof=${proof}`
      );
      if (!statusRes.ok) {
        console.error(`  Instagram poll ${i + 1}/${maxAttempts} returned ${statusRes.status}, retrying...`);
        continue;
      }
      statusData = (await statusRes.json()) as { status_code?: string };
    } catch (err) {
      console.error(`  Instagram poll ${i + 1}/${maxAttempts} failed:`, err instanceof Error ? err.message : err);
      continue;
    }
