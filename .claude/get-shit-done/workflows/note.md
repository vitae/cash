...
- **Global scope**: `C:/cash/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/cash/.claude/notes/`)
...
2. Glob `C:/cash/.claude/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `C:/cash/.claude/notes/` — works in any directory
